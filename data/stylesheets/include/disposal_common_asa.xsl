      ...
      <xsl:when test="$disposal/rda:RetentionPeriod">
        ...
        <xsl:choose>
          <xsl:when test="$disposal/rda:RetentionPeriod='1'">
            <xsl:value-of select="concat($disposal/rda:RetentionPeriod, ' ', substring-before($initialunit,'s'))"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="concat($disposal/rda:RetentionPeriod, ' ', $initialunit)"/>
          </xsl:otherwise>
        </xsl:choose>
